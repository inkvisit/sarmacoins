...
#include "config/sarmacoins-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("sarmacoins-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "sarmacoins.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "sarmacoinsd.pid") + "\n";
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in SARMACOINS/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in SARMACOINS/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           sarmacoins (or specifically: darksend, instantx, masternode, keepass, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in SARMACOINS/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all sarmacoins specific functionality (Masternodes, Darksend, InstantX, Budgeting) (0-1, default: %u)"), 0) + "\n";
    ...
    strUsage += "  -anonymizesarmacoinsamount=<n>     " + strprintf(_("Keep N SARMACOINS anonymized (default: %u)"), 0) + "\n";
...
{
    RenameThread("sarmacoins-loadblk");
...
/** Sanity checks
 *  Ensure that sarmacoins is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. sarmacoins Core is shutting down."));
...
#endif
    // Make sure only a single sarmacoins process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. sarmacoins Core is probably already running."), strDataDir));
    ...
    nAnonymizeDarkcoinAmount = GetArg("-anonymizesarmacoinsamount", 0);
    ...
    LogPrintf("Anonymize sarmacoins Amount %d\n", nAnonymizeDarkcoinAmount);
