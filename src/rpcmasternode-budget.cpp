...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if(strCommand == "prepare")
    {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start SarmaCoins_address monthly_payment_SarmaCoins'");
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SarmaCoins address");

        // Parse SarmaCoins address
    ...
    if(strCommand == "submit")
    {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start SarmaCoins_address monthly_payment_SarmaCoins fee_tx'");
